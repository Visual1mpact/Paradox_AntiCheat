    ...
    try {
        ...
        for (const player of players) {
            ...
            try {
                ...
                // Retrieve player location and dimension from the cache
                ...
                if (dimId === "minecraft:overworld" && overworld > 0) {
                    checkAndTeleportPlayer(player, dimension, x, y, z, overworld, "Overworld");
                } else if (dimId === "minecraft:nether" && nether > 0) {
                    checkAndTeleportPlayer(player, dimension, x, y, z, nether, "Nether");
                } else if (dimId === "minecraft:the_end" && end > 0) {
                    checkAndTeleportPlayer(player, dimension, x, y, z, end, "End");
                }
            } catch (e) {
...
function createWorldBorderChecker(spawnLocation: { x: number; y: number; z: number }) {
    ...
    return function checkAndTeleportPlayer(player: Player, dimension: Dimension, x: number, y: number, z: number, borderSize: number, dimensionName: string) {
        ...
        const safeY = findSafeY(player, dimension, targetX, y, targetZ);
 ...
 */
function findSafeY(player: Player, dimension: Dimension, x: number, y: number, z: number): number {
    const { min: minHeight, max: maxHeight } = getDimensionHeightRange(dimension);
