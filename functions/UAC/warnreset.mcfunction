...
tellraw @a[tag=notify] {"rawtext":[{"text":"§r§4[§6Paradox§4]§r "},{"selector":"@s"},{"text":" §1has failed §7(Exploit) §4Disabler/A §7(This check should not be used as proof)§4. VL= "},{"score":{"name":"@s","objective":"disablervl"}}]}
effect @s clear
tag @s remove admin
tag @s remove Admin
tag @s remove ADMIN
