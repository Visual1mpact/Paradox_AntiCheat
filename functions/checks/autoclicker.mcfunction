...
execute as @s[scores={attacks=8..,attack_timer=60..}] at @s run tellraw @a[tag=notify] {"rawtext":[{"text":"§r§4[§6Paradox§4]§r "},{"selector":"@s"},{"text":" §6has failed §7(Misc) §4AutoClicker/A §7(CPS: "},{"score":{"name":"@s","objective":"attacks"}},{"text":")§4 VL= "},{"score":{"name":"@s","objective":"autoclickervl"}}]}
