...
execute @s[type=player,tag=op,scores={npc=..0}] ~~~ tellraw @a[tag=op] {"rawtext":[{"text":"§r§4[§6Paradox§4]§r "},{"selector":"@s"},{"text":" has enabled §6Anti-NPC!"}]}

# allow
execute @s[type=player,tag=op,scores={npc=1..}] ~~~ scoreboard players set paradox:config npc 0
execute @s[type=player,tag=op,scores={npc=1..}] ~~~ tellraw @a[tag=op] {"rawtext":[{"text":"§r§4[§6Paradox§4]§r "},{"selector":"@s"},{"text":" has disabled §4Anti-NPC!"}]}
