...
export function autokillaura(message: BeforeChatEvent, args: string[]) {
    ...
    if (autoaurascore <= 0) {
        // Allow
        player.runCommandAsync(`scoreboard players set paradox:config autoaura 1`);
        sendMsg("@a[tag=paradoxOpped]", `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6Autoaura§r!`);
    } else if (autoaurascore >= 1) {
        // Deny
        player.runCommandAsync(`scoreboard players set paradox:config autoaura 0`);
        sendMsg("@a[tag=paradoxOpped]", `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4Autoaura§r!`);
    }
    return player.runCommandAsync(`scoreboard players operation @a autoaura = paradox:config autoaura`);
}
