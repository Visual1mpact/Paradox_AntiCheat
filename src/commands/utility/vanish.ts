 ...
 */
export async function vanish(message: ChatSendBeforeEvent, args: string[]) {
    ...
    if (vanishBoolean) {
        ...
        sendMsg(`@a[tag=paradoxOpped]`, `§r§4[§6Paradox§4]§r ${player.nameTag}§r is no longer in vanish.`);
    } else {
        player.addTag("vanish");
        player.triggerEvent("vanish");
        sendMsgToPlayer(player, `§r§4[§6Paradox§4]§r You are now vanished!`);
        sendMsg(`@a[tag=paradoxOpped]`, `§r§4[§6Paradox§4]§r ${player.nameTag}§r is now vanished!`);
    }
}
