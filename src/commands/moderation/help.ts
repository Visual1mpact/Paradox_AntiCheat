...
export function help(message: ChatSendAfterEvent) {
    ...
    return sendMsgToPlayer(player, [
        ...
        `§6${prefix}autoban§r - ${config.customcommands.tpr ? `Will ban players automaticaly if they have a violation above 50.` : textDisabled}`,
        ...
        `§6${prefix}antikillaura§r - ${config.customcommands.antikillaura ? `Toggles checks for attacks outside a 90 degree angle.` : textDisabled}`,
        ...
        `§6${prefix}reachb§r - ${config.customcommands.reachb ? `Checks if player's attack beyond reach.` : textDisabled}`,
        ...
        `§6${prefix}illegalitemsb§r - ${config.customcommands.illegalitemsb ? `Checks if player's place illegal items.` : textDisabled}`,
        `§6${prefix}illegalitemsc§r - ${config.customcommands.illegalitemsc ? `Checks for illegal dropped items in the world.` : textDisabled}`,
