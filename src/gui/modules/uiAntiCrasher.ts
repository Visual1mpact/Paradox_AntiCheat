...
export function uiANTICRASHER(anticrasherResult: ModalFormResponse, player: Player) {
    ...
    if (AntiCrasherToggle === true) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§4[§6Paradox§4]§r ${player.name}§r has enabled §6CrasherA§r!`);
        CrasherA();
    }
    if (AntiCrasherToggle === false) {
        ...
        sendMsg("@a[tag=paradoxOpped]", `§r§4[§6Paradox§4]§r ${player.name}§r has disabled §4CrasherA§r!`);
    }
