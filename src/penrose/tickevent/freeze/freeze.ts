...
function Freeze(id: number) {
    ...
    if (!hastag) {
        ...
        if (player.dimension.id === "minecraft:overworld") {
            player.runCommandAsync(`scoreboard players set @s realm 0`);
        } else if (player.dimension.id === "minecraft:nether") {
            player.runCommandAsync(`scoreboard players set @s realm 1`);
        } else if (player.dimension.id === "minecraft:the_end") {
            player.runCommandAsync(`scoreboard players set @s realm 2`);
        } else {
        ...
        player.runCommandAsync(`scoreboard players set @s xPosFreeze ${Math.floor(posx)}`);
        player.runCommandAsync(`scoreboard players set @s yPosFreeze ${Math.floor(posy)}`);
        player.runCommandAsync(`scoreboard players set @s zPosFreeze ${Math.floor(posz)}`);
        ...
        try {
            player.runCommandAsync(`fill ~1 ~2 ~1 ~-1 ~-1 ~-1 barrier 0 hollow`);
        } catch (erro) {}
        ...
        // We just need this in case they log off and log back on
        player.runCommandAsync(`scoreboard players set @s xPos1 ${Math.floor(posx1)}`);
        player.runCommandAsync(`scoreboard players set @s yPos1 ${Math.floor(posy1)}`);
        player.runCommandAsync(`scoreboard players set @s zPos1 ${Math.floor(posz1)}`);
    ...
    if (!hastag) {
        ...
        // Release from prison
        player.runCommandAsync(`fill ~1 ~2 ~1 ~-1 ~-1 ~-1 air 0 hollow`);
