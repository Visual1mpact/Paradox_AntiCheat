...
function rip(
  player: Player,
  inventory_item: ItemStack,
  enchData: { id: string; level: number },
  block: Block
) {
  if (!enchData && !block) {
    ...
    try {
      player.addTag(
        "Reason:Illegal Item C (" +
          inventory_item.id.replace("minecraft:", "") +
          "=" +
...
function illegalitemsc(object: BlockPlaceEvent) {
  ...
  let illegalEnchantmentBoolean = World.getDynamicProperty(
  ...
  if (antiShulkerBoolean && block.type.id in shulkerItems) {
    sendMsg(
      "@a[tag=notify]",
      `§r§4[§6Paradox§4]§r Removed ${block.type.id.replace(
        "minecraft:",
        ""
      )} from ${player.nameTag}.`
    );
    sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r Shulker Boxes are not allowed!`
    );
    // Set block in world
    ...
    try {
      player.runCommand(
        `fill ${x} ${y} ${z} ${x} ${y} ${z} air 0 replace air 0`
      );
    } catch (error) {}
    return;
  }
  ...
  if (
    salvageable[block.type.id] &&
    block.type.id in ignoreContainerPlace === false
  ) {
    ...
    try {
      player.runCommand(
        `fill ${x} ${y} ${z} ${x} ${y} ${z} air 0 replace ${block.id} 0`
      );
    } catch (error) {}
  ...
  if (
    block.type.id in illegalitems &&
    block.type.id in iicWhitelist === false
  ) {
    // Set block in world
    block.setType(block.type);
    // replace block in world since destroying would drop item entities
  ...
  try {
    let invComponent = block.getComponent(
      "inventory"
    ...
    for (let i = 0; i < inventory.size; i++) {
      ...
      if (salvageBoolean && inventory_item.id in whitelist === false) {
        ...
        if (verifiedItemName !== newNameTag) {
          ...
          let ench_comp = inventory_item.getComponent(
          ...
          if (ench_data) {
            for (let enchants in MinecraftEnchantmentTypes) {
              ...
              let enchant_data = ench_data.getEnchantment(
                MinecraftEnchantmentTypes[enchants]
              );
              ...
              if (
                enchantLevel &&
                enchant_data &&
                enchant_data.level <= enchantLevel &&
                enchant_data.level >= 0
              ) {
        ...
        let new_ench_comp = actualItemName.getComponent(
          "minecraft:enchantments"
        ) as ItemEnchantsComponent;
        ...
        for (let e = 0; e < enchantArray.length; e++) {
          ...
          new_ench_data.addEnchantment(
            new Enchantment(
              MinecraftEnchantmentTypes[enchantArray[e]],
              enchantLevelArray[e]
            )
          );
        ...
        if (config.debug) {
          console.warn(
            `${newNameTag} has been set and verified by Paradox (illegalitems/C)!`
          );
        }
      } else {
        /**
        ...
        if (
          salvageable[inventory_item.id] &&
          uniqueItems.indexOf(salvageable[inventory_item.id].name) !== -1 &&
          salvageable[inventory_item.id].data < inventory_item.data
        ) {
          // Reset item to data type of 0
          try {
            inventory.setItem(
              i,
              new ItemStack(Items.get(inventory_item.id), inventory_item.amount)
            );
          } catch (error) {}
          continue;
        } else if (
          ...
          try {
            inventory.setItem(
              i,
              new ItemStack(
                Items.get(inventory_item.id),
                inventory_item.amount,
                salvageable[inventory_item.id].data
              )
            );
          } catch (error) {}
          continue;
      ...
      if (inventory_item.amount < 0 || inventory_item.amount > maxStack) {
        ...
        sendMsg(
          "@a[tag=notify]",
          `§r§4[§6Paradox§4]§r ${player.nameTag}§r detected with stacked items greater than x${maxStack} for '${itemId}'.`
        );
        sendMsgToPlayer(
          player,
          `§r§4[§6Paradox§4]§r Stacked item '${itemId}' cannot exceed x${maxStack}!`
        );
        if (stackBanBoolean) {
          return rip(player, inventory_item, null, null);
        }
      ...
      if (
        illegalLoresBoolean &&
        !config.modules.illegalLores.exclude.includes(
          String(inventory_item.getLore())
        )
      ) {
        try {
          inventory.setItem(i, new ItemStack(MinecraftItemTypes.air, 0));
        ...
        sendMsg(
          ...
          `§r§4[§6Paradox§4]§r Removed ${block.type.id.replace(
            "minecraft:",
            ""
          )} with lore from ${player.nameTag}.`
        );
        sendMsgToPlayer(
          player,
          `§r§4[§6Paradox§4]§r Item with illegal lores are not allowed!`
        );
        continue;
      }
      if (illegalEnchantmentBoolean) {
        // We get a list of enchantments on this item
        let enchantComponent = inventory_item.getComponent(
          "minecraft:enchantments"
        ) as ItemEnchantsComponent;
          ...
          for (let {
            ...
            if (!enchantLevel) {
              ...
              sendMsg("@a[tag=notify]", [
                `§r§4[§6Paradox§4]§r §4[§f${
                  player.nameTag
                }§4]§r §6=>§r §4[§f${block.type.id.replace(
                  "minecraft:",
                  ""
                )}§4]§r §6=>§r §4[§fSlot§4]§r ${i}§r §6=>§r §4[§f${inventory_item.id.replace(
                  "minecraft:",
                  ""
                )}§4]§r §6Enchanted: §4${id}=${level}§r`,
                `§r§4[§6Paradox§4]§r Removed §4[§f${inventory_item.id.replace(
                  "minecraft:",
                  ""
                )}§4]§r from ${player.nameTag}.`,
              ]);
              sendMsgToPlayer(
                player,
                `§r§4[§6Paradox§4]§r Illegal enchantments are not allowed!`
              );
      ...
      if (block.type.id in whitelistChest) {
        ...
        sendMsg(
          ...
          `§r§4[§6Paradox§4]§r ${
            player.nameTag
          } placed a nested chest at X=${x.toFixed(0)}, Y=${y.toFixed(
            0
          )}, Z=${z.toFixed(0)}. Chest has been cleared!`
        );
        sendMsgToPlayer(
          player,
          `§r§4[§6Paradox§4]§r Nested chests are not allowed. This chest has been cleared!`
        );
