...
const ChatFilter = () => {
  World.events.beforeChat.subscribe((msg) => {
    ...
    if (msg.sender.hasTag("tpr") && msg.message.toLowerCase().includes("yes")) {
      ...
      for (let player of World.getPlayers()) {
        if (player.hasTag("tpr:" + msg.sender.name)) {
          ...
          if (cooldownCalc === msSettings || cooldownCalc <= 0) {
            ...
            sendMsgToPlayer(
              player,
              `§r§4[§6Paradox§4]§r Hello ${player.name}! ${msg.sender.name} has approved your request to teleport to their location!`
            );
            sendMsgToPlayer(
              msg.sender,
              `§r§4[§6Paradox§4]§r You have approved a teleport request from ${player.name}!`
            );
      ...
      msg.sender.hasTag("tpr") &&
      msg.message.toLowerCase().includes("no")
    ) {
      msg.cancel = true;
      for (let player of World.getPlayers()) {
        if (player.hasTag("tpr:" + msg.sender.name)) {
          ...
          if (cooldownCalc === msSettings || cooldownCalc <= 0) {
            ...
            sendMsgToPlayer(
              player,
              `§r§4[§6Paradox§4]§r Hello ${player.name}! ${msg.sender.name} has denied your request to teleport to their location!`
            );
            sendMsgToPlayer(
              msg.sender,
              `§r§4[§6Paradox§4]§r You have denied a teleport request from ${player.name}!`
            );
            player.removeTag("tpr:" + msg.sender.name);
            msg.sender.removeTag("tpr");
            break;
    ...
    if (chatRanksBoolean === true) {
      ...
      if (!msg.cancel) {
        if (rcbrBoolean)
          sendMsg(
            "RealmBot",
            "RB_COMMAND" +
              `{content:'[§6${rank}§4§r] §7${player.name}: §r${message}'}`
          );
      ...
      if (rcbrBoolean)
        sendMsg(
          "RealmBot",
          "RB_COMMAND" + `{content:'${player.name}: ${message}'}`
        );
