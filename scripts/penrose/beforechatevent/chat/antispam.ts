...
const spamList = new WeakMap<
  Player,
  { lastSendTime: number; lastMessage: string; spamCounter: number }
>();
...
function antispam(msg: BeforeChatEvent) {
  ...
  if (hash === undefined || encode !== hash) {
    ...
    // resets spam data
    if (
      currentTime - spamData.lastSendTime >=
      config.modules.antispam.cooldown * 50
    ) {
      check = false;
    ...
    if (check) {
      ...
      // check if the player sends a message during cooldown
        ...
        sendMsgToPlayer(
          player,
          `§r§4[§6Paradox§4]§r You are sending too many messages in a short time!`
        );
      }
    }
