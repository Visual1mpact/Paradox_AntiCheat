...
function worldborder() {
  ...
  let worldBorderNetherNumber = World.getDynamicProperty(
  ...
  for (let player of World.getPlayers()) {
    ...
    if (
      (overworldSize === 0 && player.dimension.id === "minecraft:overworld") ||
      (netherSize === 0 && player.dimension.id === "minecraft:nether")
    ) {
      continue;
    }
     ...
     */
    if (
      MinecraftBlockTypes.portal.id in portals ||
      portals[0] === MinecraftBlockTypes.air.id
    ) {
      continue;
    }
    ...
    if (player.dimension.id === "minecraft:overworld") {
      ...
      if (
        player.location.x > overworldSize ||
        player.location.x < -overworldSize ||
        player.location.z > overworldSize ||
        player.location.z < -overworldSize
      ) {
        sendMsgToPlayer(
          player,
          `§r§4[§6Paradox§4]§r You have reached the world border.`
        );
        // Positives
        if (
          player.location.x >= overworldSize &&
          ...
          player.teleport(
            new Location(overworldSize - 3, safe, overworldSize - 3),
            player.dimension,
            0,
            0
          );
        ...
        if (player.location.x >= overworldSize) {
          ...
          player.teleport(
            new Location(overworldSize - 3, safe, player.location.z),
            player.dimension,
            0,
            0
          );
        ...
        if (player.location.z >= overworldSize) {
          ...
          player.teleport(
            new Location(player.location.x, safe, overworldSize - 3),
            player.dimension,
            0,
            0
          );
    ...
    if (player.dimension.id === "minecraft:nether") {
      // Make sure nobody climbs over the wall
      if (
        player.location.x > netherSize ||
        player.location.x < -netherSize ||
        player.location.z > netherSize ||
        player.location.z < -netherSize
      ) {
        sendMsgToPlayer(
          player,
        ...
        if (
          ...
          player.teleport(
            new Location(netherSize - 3, player.location.y, netherSize - 3),
            player.dimension,
            0,
            0
          );
          ...
          player.teleport(
            new Location(netherSize - 3, safe, netherSize - 3),
            player.dimension,
            0,
            0
          );
        ...
        if (player.location.x >= netherSize) {
          ...
          player.teleport(
            new Location(netherSize - 3, player.location.y, player.location.z),
            player.dimension,
            0,
            0
          );
          ...
          player.teleport(
            new Location(netherSize - 3, safe, player.location.z),
            player.dimension,
            0,
            0
          );
        ...
        if (player.location.z >= netherSize) {
          ...
          player.teleport(
            new Location(player.location.x, player.location.y, netherSize - 3),
            player.dimension,
            0,
            0
          );
          ...
          player.teleport(
            new Location(player.location.x, safe, netherSize - 3),
            player.dimension,
            0,
            0
          );
