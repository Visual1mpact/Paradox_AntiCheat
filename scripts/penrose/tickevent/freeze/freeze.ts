...
function Freeze() {
  ...
  if (!hastag) {
    ...
    if (player.dimension.id === "minecraft:overworld") {
      ...
      sendMsg(
        "@a[tag=paradoxOpped]",
        `§r§4[§6Paradox§4]§r Cannot determine dimension for ${player.nameTag}.`
      );
    ...
    player.runCommand(
      `scoreboard players set @s xPosFreeze ${Math.floor(posx)}`
    );
    player.runCommand(
      `scoreboard players set @s yPosFreeze ${Math.floor(posy)}`
    );
    player.runCommand(
      `scoreboard players set @s zPosFreeze ${Math.floor(posz)}`
    );
  ...
  if (posx1 !== posx || posy1 !== posy || posz1 !== posz) {
    ...
    try {
      player.teleport(
        new Location(posx1, posy1, posz1),
        World.getDimension("overworld"),
        0,
        0
      );
  ...
  if (!hastag) {
    ...
    player.teleport(
      new Location(backx, backy, backz),
      World.getDimension(realmIDString),
      0,
      0
    );
    player.removeTag("freezeactive");
