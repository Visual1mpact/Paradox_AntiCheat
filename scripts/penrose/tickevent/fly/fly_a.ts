...
function flya(id: number) {
  ...
  for (let player of World.getPlayers(gm)) {
    ...
    let xyVelocity = Math.hypot(player.velocity.x, player.velocity.y).toFixed(
      4
    );
    let zyVelocity = Math.hypot(player.velocity.z, player.velocity.y).toFixed(
      4
    );
    ...
    try {
      ...
      Block1 = player.dimension.getBlock(
        new BlockLocation(
          player.location.x,
          player.location.y - 1,
          player.location.z
    ...
    if (player.hasTag("ground")) {
      ...
      playersOldCoordinates.set(player.name, {
        x: playerX,
        y: playerY,
        z: playerZ,
      });
