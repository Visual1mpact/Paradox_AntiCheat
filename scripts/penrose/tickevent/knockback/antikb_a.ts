...
function antiknockbacka(id: number) {
  ...
  for (let player of World.getPlayers()) {
    ...
    if (
      Number(
        (player.velocity.y + player.velocity.x + player.velocity.z).toFixed(3)
      ) <= config.modules.antikbA.magnitude
    ) {
      if (
        player.hasTag("attacked") &&
        !player.hasTag("dead") &&
        ...
        try {
          ...
          flag(
            ...
            null,
            "Magnitude",
            (player.velocity.y + player.velocity.x + player.velocity.z).toFixed(
              3
            ),
            true,
            null
