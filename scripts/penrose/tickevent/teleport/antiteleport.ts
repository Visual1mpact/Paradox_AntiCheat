  ...
  for (let player of World.getPlayers()) {
    ...
    if (
      ("minecraft:portal" ||
        ...
        "minecraft:end_gateway") in portals
    ) {
    ...
    if (
      xPos > config.modules.antiTeleport.constraint ||
      zPos > config.modules.antiTeleport.constraint ||
      xPos < -config.modules.antiTeleport.constraint ||
      zPos < -config.modules.antiTeleport.constraint
    ) {
      if (portals[5] === "minecraft:air") {
        ...
        if (teleportScore === 0) {
          ...
          player.teleport(
            new Location(xScore, yScore, zScore),
            player.dimension,
            0,
            0
          );
          try {
            sendMsg(
              ...
              `§r§4[§6Paradox§4]§r ${
                player.nameTag
              } §6has failed §7(Movement) §4Teleport/A. VL= ${getScore(
                "tpvl",
                player
              )}`
            );
