  ...
  for (let player of World.getPlayers()) {
    ...
    for (let i = 0; i < container.size; i++) {
      ...
      if (antiShulkerBoolean && inventory_item.id in shulkerItems) {
        ...
        sendMsg(
          "@a[tag=notify]",
          `§r§4[§6Paradox§4]§r Removed ${inventory_item.id.replace(
            "minecraft:",
            ""
          )} from ${player.nameTag}.`
        );
      ...
      if (salvageBoolean && inventory_item.id in whitelist === false) {
        ...
        if (verifiedItemName !== newNameTag) {
          ...
          if (config.debug) {
            console.warn(
              `${newNameTag} has been set and verified by Paradox (illegalitems/A)!`
            );
          }
        }
      } else {
      ...
      if (illegalEnchantmentBoolean) {
        ...
        let enchantContainer = inventory_item.getComponent(
          "minecraft:enchantments"
        ) as ItemEnchantsComponent;
