...
function vanish() {
  ...
  for (let player of World.getPlayers(filter)) {
    ...
    if (hash === undefined || encode !== hash) {
      ...
      if (
        player.getEffect(MinecraftEffectTypes.invisibility) ||
        player.getEffect(MinecraftEffectTypes.nightVision)
      ) {
      ...
      try {
        sendMsg(
          "@a[tag=notify]",
          `§r§4[§6Paradox§4]§r ${player.nameTag} had unauthorized permissions for Vanish. Permissions removed!`
        );
      } catch (error) {}
    }
  }
