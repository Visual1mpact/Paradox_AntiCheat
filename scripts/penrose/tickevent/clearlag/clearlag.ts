...
function executionItem() {
  ...
  for (let entity of World.getDimension("overworld").getEntities(filter)) {
    ...
    try {
      let itemContainer = entity.getComponent(
        "item"
      ) as unknown as EntityItemComponent;
...
function clearlag(id: number) {
  ...
  let msSettings =
    config.modules.clearLag.days * 24 * 60 * 60 * 1000 +
    config.modules.clearLag.hours * 60 * 60 * 1000 +
    config.modules.clearLag.minutes * 60 * 1000 +
    config.modules.clearLag.seconds * 1000;
  ...
  if (
    activeTimer[0] === 1 &&
    activeTimer[2] === "minutes" &&
    activeTimer[1] <= 0
  ) {
    // Notify 10 seconds in advance
    sendMsg(
      "@a",
      `§r§4[§6Paradox§4]§r Server lag will be cleared in 60 seconds!`
    );
  }
  // Give advance warning
