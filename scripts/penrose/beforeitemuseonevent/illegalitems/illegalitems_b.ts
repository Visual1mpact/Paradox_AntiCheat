...
function illegalitemsb(object: BeforeItemUseOnEvent) {
  ...
  if (config.debug) {
    source.runCommand(
      `say Item: ${item.id}, Data: ${item.data}, Amount: ${item.amount}`
    );
  }
  ...
  if (antiShulkerBoolean && item.id in shulkerItems) {
    object.cancel = true;
    let invContainer = source.getComponent(
      "minecraft:inventory"
    ) as EntityInventoryComponent;
    invContainer.container.setItem(
      hand,
      new ItemStack(MinecraftItemTypes.air, 0)
    );
    sendMsg(
      "@a[tag=notify]",
      `§r§4[§6Paradox§4]§r Removed ${item.id.replace("minecraft:", "")} from ${
        source.nameTag
      }.`
    );
    sendMsgToPlayer(
  ...
  if (salvageBoolean && item.id in whitelist === false) {
    ...
    try {
      ...
      if (verifiedItemName !== newNameTag) {
        ...
        let ench_comp = item.getComponent(
        ...
        if (config.debug) {
          console.warn(
            `${newNameTag} has been set and verified by Paradox (illegalitems/B)!`
          );
        }
      }
    } catch (error) {}
    ...
    if (
      salvageable[item.id] &&
      uniqueItems.indexOf(salvageable[item.id].name) !== -1 &&
      salvageable[item.id].data < item.data
    ) {
      ...
      try {
        ...
        invContainer.container.setItem(
          hand,
          new ItemStack(Items.get(item.id), item.amount)
        );
      } catch (error) {}
      return;
    } else if (
      ...
      try {
        ...
        invContainer.container.setItem(
          hand,
          new ItemStack(Items.get(item.id), item.amount, item.data)
        );
      } catch (error) {}
      return;
    }
  ...
  if (item.amount < 0 || item.amount > maxStack) {
    ...
    try {
      ...
      sendMsg(
        "@a[tag=notify]",
        `§r§4[§6Paradox§4]§r ${source.nameTag}§r detected with stacked items greater than x${maxStack} for '${itemId}'.`
      );
      sendMsgToPlayer(
        source,
  ...
  if (
    illegalLoresBoolean &&
    !config.modules.illegalLores.exclude.includes(String(item.getLore()))
  ) {
    object.cancel = true;
    try {
      let invContainer = source.getComponent(
    ...
    sendMsg(
      "@a[tag=notify]",
      `§r§4[§6Paradox§4]§r Removed ${item.id.replace(
        "minecraft:",
        ""
      )} with lore from ${source.nameTag}.`
    );
    sendMsgToPlayer(
      source,
  ...
  if (illegalEnchantmentBoolean) {
    ...
    let enchantComponent = item.getComponent(
      "minecraft:enchantments"
    ) as ItemEnchantsComponent;
        ...
        if (!enchantLevel) {
          ...
          sendMsg("@a[tag=notify]", [
            `§r§4[§6Paradox§4]§r §4[§f${
              source.nameTag
            }§4]§r §6=>§r §4[§fSlot§4]§r ${hand}§r §6=>§r §4[§f${item.id.replace(
              "minecraft:",
              ""
            )}§4]§r §6Enchanted: §4${id}=${level}§r`,
            `§r§4[§6Paradox§4]§r Removed §4[§f${item.id.replace(
              "minecraft:",
              ""
            )}§4]§r from ${source.nameTag}.`,
          ]);
          sendMsgToPlayer(
            source,
