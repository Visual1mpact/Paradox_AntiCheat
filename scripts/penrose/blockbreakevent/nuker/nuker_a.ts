...
function nukera(object: BlockBreakEvent) {
  ...
  let invContainer = player.getComponent(
    "minecraft:inventory"
  ...
  if (item_enchants.hasEnchantment(MinecraftEnchantmentTypes.efficiency)) {
    enchantment = item_enchants.getEnchantment(
  ...
  if (
    enchantment &&
    enchantment.level >= MinecraftEnchantmentTypes.efficiency.maxLevel &&
    tiktok.length < config.modules.antinukerA.max + 2
  ) {
    return undefined;
  }
  ...
  if (
    tiktok.length >= config.modules.antinukerA.max &&
    vegetation.indexOf(brokenBlockPermutation.type.id) === -1
  ) {
