...
const execCmd = (
  command: string,
  source: Dimension | Player = overworld,
  ignoreError = false
...
class objective {
  static readonly create = (id: string, displayName = id) =>
    new this(id, displayName, true);
  static readonly edit = (id: string) => new this(id, id, false);
  static readonly for = (id: string, displayName = id) =>
    new this(id, displayName, !this.exist(id));
  ...
  static readonly delete = (id: string) =>
    !execCmd(
      `scoreboard objectives remove ${toExecutable(id)}`,
      undefined,
      true
    ).statusCode;
  static readonly getList = () =>
    world.scoreboard.getObjectives().map((v) => this.edit(v.id));

  constructor(id: string, displayName = id, create = true) {
    if (id.length > 16)
      throw new RangeError(
        `Objective identifier length cannot go more than 16 characters`
      );
    if (displayName.length > 32)
      throw new RangeError(
        `Objective display length cannot go more than 32 characters`
      );
...
class players {
  ...
  readonly "set" = (plr: Player, score: number) => (
    void execCmd(
      `scoreboard players set @s ${this.#obj.execId} ${score}`,
      plr,
      true
    ),
    this
  );
  readonly add = (plr: Player, score: number) => (
    void execCmd(
      `scoreboard players add @s ${this.#obj.execId} ${score}`,
      plr,
      true
    ),
    this
  ...
  readonly "get" = (plr: Player) => {
    const r = execCmd(
      `scoreboard players test @s ${this.#obj.execId} * *`,
      plr,
      true
    );
  ...
  readonly exist = (plr: Player) =>
    !execCmd(`scoreboard players test @s ${this.#obj.execId} * *`, plr, true)
      .statusCode;
  readonly delete = (plr: Player) =>
    !execCmd(`scoreboard players reset @s ${this.#obj.execId}`, plr, true)
      .statusCode;
  readonly getScores = (() => {
    const t = this;
    return function* () {
      for (const { participant, score } of t.#data.getScores())
        if (participant.type == ScoreboardIdentityType.player)
          try {
            yield [participant.getEntity(), score, participant.displayName] as [
              player: Player,
              score: number,
              displayName: string
            ];
          } catch {
            yield [null, score, null] as [
              player: null,
              score: number,
              displayName: null
            ];
...
class dummies {
  ...
  readonly "set" = (name: string, score: number) => (
    void execCmd(
      `scoreboard players set ${toExecutable(name)} ${
        this.#obj.execId
      } ${score}`,
      undefined,
      true
  ...
  readonly add = (name: string, score: number) => (
    void execCmd(
      `scoreboard players add ${toExecutable(name)} ${
        this.#obj.execId
      } ${score}`,
      undefined,
      true
  ...
  readonly "get" = (name: string) => {
    const r = execCmd(
      `scoreboard players test ${toExecutable(name)} ${this.#obj.execId} * *`,
      undefined,
      true
    );
    if (r.statusCode) return undefined;
  ...
  readonly delete = (name: string) =>
    !execCmd(
      `scoreboard players reset ${toExecutable(name)} ${this.#obj.execId}`,
      undefined,
      true
    ).statusCode;
  readonly getScores = (() => {
    ...
    return function* () {
      for (const { participant, score } of t.#data.getScores())
        if (participant.type == ScoreboardIdentityType.fakePlayer)
          yield [participant.displayName, score] as [
            displayName: string,
            score: number
          ];
    };
...
class display {
  static readonly setDisplay = (
    displaySlot: displaySlot,
    scoreboard: objective | string
  ) =>
    !execCmd(
      `scoreboard objectives setdisplay ${displaySlot} ${
        scoreboard instanceof objective
          ? scoreboard.execId
          : toExecutable(scoreboard)
      }`,
      undefined,
      true
    ).statusCode;
  static readonly clearDisplay = (displaySlot: displaySlot) =>
    !execCmd(`scoreboard objectives setdisplay ${displaySlot}`, undefined, true)
      .statusCode;
  ...
  readonly setDisplay = (displaySlot: displaySlot) =>
    !execCmd(
      `scoreboard objectives setdisplay ${displaySlot} ${this.#obj.execId}`,
      undefined,
      true
    ).statusCode;
