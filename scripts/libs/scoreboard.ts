        ...
        return function* () {
            for (const { participant, score } of t.#data.getScores())
                if (participant.type == ScoreboardIdentityType.player)
                    try {
                        yield [participant.getEntity(), score, participant.displayName] as [player: Player, score: number, displayName: string];
                    } catch {
                        yield [null, score, null] as [player: null, score: number, displayName: null];
                    }
        };
    })();
}
...
class dummies {
    ...
    readonly getScores = (() => {
        const t = this;
        return function* () {
            for (const { participant, score } of t.#data.getScores()) if (participant.type == ScoreboardIdentityType.fakePlayer) yield [participant.displayName, score] as [displayName: string, score: number];
        };
    })();
}

class display {
    static readonly setDisplay = (displaySlot: displaySlot, scoreboard: objective | string) =>
        !execCmd(`scoreboard objectives setdisplay ${displaySlot} ${scoreboard instanceof objective ? scoreboard.execId : toExecutable(scoreboard)}`, undefined, true).statusCode;
