...
if (config.debug) {
  for (const v of [Entity, Player, World]) {
    ...
    v.prototype.getDynamicProperty = function (id) {
      console.log(
        `Getting dynamic property '${id}' of ${
      ...
      try {
        ...
        console.warn(
          `Get dynamic property '${id}' of ${this.constructor.name} FAILED: \n${e}`
        );
        throw e;
      }
    };
    ...
    v.prototype.setDynamicProperty = function (id, v) {
      console.log(
        `Setting dynamic property '${id}' of ${
          this.constructor.name
        } to '${v}' (${typeof v}) \n${getStack()}`
      );
      try {
        ...
        console.warn(
          `Set dynamic property '${id}' of ${
            this.constructor.name
          } to '${v}' (${typeof v}) FAILED: \n${e}`
        );
        throw e;
      }
    };
    ...
    v.prototype.removeDynamicProperty = function (id) {
      console.log(
        `Deleting dynamic property '${id}' of ${
          this.constructor.name
        } \n${getStack()}`
      );
      try {
        ...
        console.warn(
          `Delete dynamic property '${id}' of ${this.constructor.name} FAILED: \n${e}`
        );
        throw e;
      }
    };
  ...
  PropertyRegistry.prototype.registerEntityTypeDynamicProperties = function (
    def,
    type
  ) {
    console.log(
      `Registering dynamic properties definition to entity type '${
        type.id
      }' \n${getStack()}`
    );
    try {
      ...
      console.warn(
        `Register dynamic properties definition to entity type '${type.id}' FAILED: \n${e}`
      );
      throw e;
    }
  };
  ...
  PropertyRegistry.prototype.registerWorldDynamicProperties = function (def) {
    console.log(
      `Registering dynamic properties definition to world \n${getStack()}`
    );
    try {
      regWld.call(this, def);
    } catch (e) {
      console.warn(
        `Register dynamic properties definition to world FAILED: \n${e}`
      );
      throw e;
    }
  };
