...
const viewObj = (() => {
    ...
    const exec = (o: { [x: string]: any; replace: (arg0: RegExp, arg1: ([v, a]: [any, any]) => string) => any; length: number; size: any; }, stack: any[], tab: string, tabLevel: number, tabSeparator: string) => {
        ...
        if (o == null)
            return `§8${o}§r`;
        const nStack = stack.concat([o]), cTab = tab.repeat(tabLevel), nTab = tab.repeat(tabLevel + 1), nTabLvl = tabLevel + 1, execNext = (k: string | number | symbol, obj: undefined) => {
            try {
        ...
        switch (oc) {
            ...
            default: {
                let name = oc == null ? `[${o[String(Symbol.toStringTag)] ?? 'Object'}: null prototype]`
                    : oc != Object ? oc.name
                        : Symbol.toStringTag in o ? `Object [${o[String(Symbol.toStringTag)]}]`
