...
const viewObj = (() => {
  ...
  Object.defineProperty(AsyncGeneratorObjCst, "name", {
  ...
  const fnHead = (
    o: { name: string; fileName: any; lineNumber: any },
    oc: Function
  ) => {
    const fName = o.name || "<anonymous>",
      fLoc = o.fileName ? `${o.fileName}:${o.lineNumber}` : "<native>",
      notClass =
        Object.getOwnPropertyDescriptor(o, "prototype")?.writable ?? true,
      asyncText =
        oc == AsyncFunction || oc == AsyncGeneratorFunction ? "Async" : "",
      generatorText =
        oc == GeneratorFunction || oc == AsyncGeneratorFunction
          ? "Generator"
          : "",
  ...
  const getKeys = (
    o: object,
    op = Object.getPrototypeOf(o),
    getPrototypeKeys = true,
    excludeKeys = []
  ) => {
    ...
    return Array.from(keysSet, (k) => {
      const descriptor =
          ...
          Object.getOwnPropertyDescriptor(op, k),
  ...
  const exec = (
    o: {
      [x: string]: any;
      replace: (arg0: RegExp, arg1: ([v, a]: [any, any]) => string) => any;
      length: number;
      size: any;
    },
    stack: any[],
    ...
    switch (oc) {
      ...
      case Array: {
        ...
        for (const k in o) {
          exclude[k] = 0;
          out.push(
            `${nTab}${tabSeparator}${formatKey(k)}: ${execNext(k, null)}`
          );
        }
        for (let i = 0; i < o.length; i++)
          if (i in o && !(i in exclude))
            out.push(
              `${nTab}${tabSeparator}${i} §7[F]§r : ${execNext(i, null)}`
            );
        out.push(`${cTab}${tabSeparator}]`);
        return out.join("\n");
      }
      ...
      case Map: {
        ...
        for (const [k, v] of o as any)
          out.push(
            `${nTab}${tabSeparator}§l=>§r ${execNext(null, k)} -> ${execNext(
              null,
              v
            )}`
          );
      ...
      default: {
        ...
        for (const { key, isGet, isSet } of keys)
          out.push(
            `${nTab}${tabSeparator}${formatKey(key, isGet, isSet)}: ${execNext(
              key,
              null
            )}`
          );
  ...
  return (o: any, tab = " §8:§r ", tabSeparator = " ") =>
    exec(o, [], tab, 0, tabSeparator);
})();
