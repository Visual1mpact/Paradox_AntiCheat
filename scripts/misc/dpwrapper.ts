...
if (config.dynamicPropertyWrapper.enabled) {
  ...
  const getPropertyDatas = (scoreObj: scoreboardObjectiveDummies) => {
    const data: Record<
      string,
      { cachedValue: string | number | boolean; scoreboardName: string }
    > = Object.create(null);
    for (const [name] of scoreObj.getScores()) {
      // parse the saved value
      const [match, id, type] =
        name.match(/(.*?)\|\|(string|number|boolean)\|\|/) ?? [];
  ...
  SimulatedPlayer.prototype.getDynamicProperty =
    Player.prototype.getDynamicProperty;
  SimulatedPlayer.prototype.setDynamicProperty =
    Player.prototype.setDynamicProperty;
  SimulatedPlayer.prototype.removeDynamicProperty =
    Player.prototype.removeDynamicProperty;

  // player data & uid registry
  const staticUidRegistry = scoreboard.objective.for(
    `${uniqueID}:uidreg`
  ).players;
  ...
  const getDataOfPlayer = (plr: Player) => {
    ...
    const obj = scoreboard.objective.for(
      `${uniqueID}:plr:${plr.scoreboard.id.toString(36)}`
    ).dummies;
