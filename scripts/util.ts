...
export function flag(
  ...
  debug: string,
  shouldTP: boolean,
  message: BeforeChatEvent
) {
  // make sure the vl objective exists
  try {
    player.runCommand(
      `scoreboard objectives add ${check.toLowerCase()}vl dummy`
    );
  } catch (error) {}
  ...
  if (shouldTP && check !== "Crasher") {
    ...
    player.teleport(
      new Location(30000000, 30000000, 30000000),
      player.dimension,
      0,
      0
    );
  ...
  try {
    if (debug) {
      sendMsg(
        ...
        `§r§4[§6Paradox§4]§r ${
          player.nameTag
        } §6has failed §7(${hackType}) §4${check}/${checkType} §7(${debugName}=${debug})§4. VL= ${getScore(
          check.toLowerCase() + "vl",
          player
        )}`
      );
    } else if (item && stack) {
      sendMsg(
        ...
        `§r§4[§6Paradox§4]§r ${
          player.nameTag
        } §6has failed §7(${hackType}) §4${check}/${checkType} §7(${item.replace(
          "minecraft:",
          ""
        )}=${stack})§4. VL= ${getScore(check.toLowerCase() + "vl", player)}`
      );
    } else {
      sendMsg(
        "@a[tag=notify]",
        `§r§4[§6Paradox§4]§r ${
          player.nameTag
        } §6has failed §7(${hackType}) §4${check}/${checkType}. VL= ${getScore(
          check.toLowerCase() + "vl",
          player
        )}`
      );
  ...
  try {
    if (check === "Namespoof") {
      player.runCommand(
        `kick ${JSON.stringify(
          player.name
        )} §r§4[§6Paradox§4]§r Please use your real Xbox profile name!`
      );
    }
  } catch (error) {
...
export function getScore(
  ...
  try {
    const data = player.runCommand(
      `scoreboard players test @s ${objective} ${minimum ? minimum : "*"} ${
        maximum ? maximum : "*"
      }`
    );
...
export function resetTag(member: Player) {
  ...
  sendMsg(
    "@a[tag=paradoxOpped]",
    `§r§4[§6Paradox§4]§r ${member.nameTag} has reset their rank`
  );
}
...
export const crypto = (salt: string | number | boolean, text: string) => {
  ...
  const applySaltToChar = (code: number[]) =>
    textToChars(String(salt)).reduce(
