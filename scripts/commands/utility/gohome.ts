...
export function gohome(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? ./commands/utility/gohome.js:52)"
    );
  }
  ...
  if (!homex || !homey || !homez || !dimension) {
    sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r Home '${args[0]}' does not exist!`
    );
  } else {
    ...
    if (
      ...
      sendMsgToPlayer(
        player,
        `§r§4[§6Paradox§4]§r Too fast! Please wait for ${activeTimer} before going home.`
      );
    }
  }
}
