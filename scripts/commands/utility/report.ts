...
export function report(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? ./commands/moderation/ban.js:29)"
    );
  }
  ...
  if (member === player) {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r You cannot report yourself.`
    );
  }

  sendMsgToPlayer(
    player,
    `§r§4[§6Paradox§4]§r Reported ${member.nameTag}§r with reason: ${reason}`
  );

  sendMsg(
    "@a[tag=notify]",
    `§r§4[§6Paradox§4]§r ${player.nameTag}§r has reported ${member.nameTag}§r with reason: ${reason}`
  );
}
