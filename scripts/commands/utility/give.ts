...
export function give(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/utility/give.js:36)"
    );
  }
  ...
  if (confirmItem) {
    ...
    const maxStack =
      maxItemStack[
        itemStringConvert.replace(itemStringConvert, "minecraft:" + args[1])
      ] ?? defaultMaxItemStack;
    if (maxStack >= Number(args[2])) {
      let invContainer = member.getComponent(
        "inventory"
      ) as EntityInventoryComponent;
      let inv = invContainer.container;
      let item = new ItemStack(
        MinecraftItemTypes[itemStringConvert],
        Number(args[2]),
        Number(args[3])
      );
      inv.addItem(item);
      return sendMsgToPlayer(
        player,
        `§r§4[§6Paradox§4]§r ${member.name} was given ${args[1]} x${args[2]}.`
      );
    } else {
      return sendMsgToPlayer(
        player,
        `§r§4[§6Paradox§4]§r This stack is too high! ${maxStack} is the max. Try again.`
      );
    }
  } else {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r This item could not be found! Try again.`
    );
  }
}
