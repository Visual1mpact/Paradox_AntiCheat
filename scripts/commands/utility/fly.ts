...
function mayflydisable(player: Player, member: Player) {
  sendMsg(
    "@a[tag=paradoxOpped]",
    `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled fly mode for ${
      player === member ? "themselves" : member.nameTag
    }.`
  );
}
...
function mayflyenable(player: Player, member: Player) {
  sendMsg(
    "@a[tag=paradoxOpped]",
    `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled fly mode for ${
      player === member ? "themselves" : member.nameTag
    }.`
  );
}
...
export function fly(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/utility/fly.js:38)"
    );
  }
  ...
  if (!membertag.includes("noflying") && !membertag.includes("flying")) {
    try {
      ...
      return sendMsgToPlayer(
        player,
        `§r§4[§6Paradox§4]§r Education Edition is disabled in this world.`
      );
    }
    return;
  }
