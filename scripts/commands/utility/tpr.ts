...
export function tpr(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/utility/tpr.js:56)"
    );
  }
  ...
  if (argCheck && args[0].toLowerCase() === "block") {
    ...
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r You have blocked all future teleport requests!`
    );
  } else if (argCheck && args[0].toLowerCase() === "unblock") {
    player.removeTag("tprblock");
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r You have unblocked all future teleport requests!`
    );
  }
  ...
  if (member.hasTag("tprblock")) {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r This player has all teleport requests blocked!`
    );
  }
  ...
  if (player.name === member.name) {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r You cannot send a teleport request to yourself! Try again.`
    );
  }
  ...
  if (member.hasTag("tpr")) {
    ...
    if (cooldownCalc === msSettings || cooldownCalc <= 0) {
      ...
      sendMsgToPlayer(
        player,
        `§r§4[§6Paradox§4]§r Hello ${player.name}! Your teleport request has been sent to ${member.name}.`
      );
      return sendMsgToPlayer(
        member,
        `§r§4[§6Paradox§4]§r Hello ${member.name}! ${player.name} is requesting to teleport to your location! Type yes or no.`
      );
    } else {
      return sendMsgToPlayer(
        player,
        `§r§4[§6Paradox§4]§r ${member.name} is pending a request. Try again later.`
      );
    }
  } else {
    member.addTag("tpr");
