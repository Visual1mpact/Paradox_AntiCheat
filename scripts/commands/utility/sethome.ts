...
export function sethome(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? ./commands/utility/sethome.js:26)"
    );
  }
  ...
  for (let i = 0; i < tags.length; i++) {
    if (tags[i].startsWith(args[0].toString() + " X", 13)) {
      ...
      sendMsgToPlayer(
        player,
        `§r§4[§6Paradox§4]§r Home with name '${args[0]}' already exists!`
      );
      break;
    }
    ...
    if (
      ...
      sendMsgToPlayer(
        player,
        `§r§4[§6Paradox§4]§r You can only have ${config.modules.setHome.max} saved locations at a time!`
      );
      break;
    }
  }
  ...
  if (player.dimension.id === "minecraft:the_end") {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r Not allowed to set home in this dimension!`
    );
  }
  ...
  sendMsgToPlayer(
    player,
    `§r§4[§6Paradox§4]§r Home '${args[0]}' has been set at ${homex} ${homey} ${homez}!`
  );
}
