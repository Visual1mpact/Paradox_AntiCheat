...
export function invsee(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? ./commands/utility/invsee.js:30)"
    );
  }
  ...
  sendMsgToPlayer(player, [
    ...
    ...Array.from(Array(container.size), (_a, i) => {
      ...
      return ` | §fSlot ${i}§r §6=>§r ${
        item
          ? `§4[§f${item.id.replace("minecraft:", "")}§4]§r §6Amount: §4x${
              item.amount
            }§r §6=>§r §4[§fData ${item.data}§4]§r`
          : "§7(empty)"
      }`;
    }),
    ` `,
