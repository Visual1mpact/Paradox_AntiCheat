...
export function tag(message: BeforeChatEvent, args: string[]) {
	...
	if (!message) {
		return console.warn(
			`${new Date()} | ` +
				"Error: ${message} isnt defined. Did you forget to pass it? (./utility/tag.js:37)"
		);
	}
	...
	if (player === member) {
		return sendMsg(
			"@a[tag=paradoxOpped]",
			`${player.nameTag} has changed their rank`
		);
	}

	sendMsg(
		"@a[tag=paradoxOpped]",
		`${player.nameTag} has changed ${member.nameTag}'s rank!`
	);
}
