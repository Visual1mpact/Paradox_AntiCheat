...
export function clearchat(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./utility/notify.js:26)"
    );
  }
  ...
  if (
    ...
    !config.customcommands.clearchat
  ) {
  ...
  sendMsg(
    "@a[tag=paradoxOpped]",
    `§r§4[§6Paradox§4]§r Chat has been cleared by ${player.nameTag}`
  );
}
