...
export function freeze(message, args) {
    ...
    if (!args.length) return player.dimension.runCommand(`tellraw "${player.nameTag}" {"rawtext":[{"text":"§r§4[§6Paradox§4]§r "},{"text":"You need to provide which target to freeze!"}]}`);
    ...
    return player.dimension.runCommand(`execute "${member.nameTag}" ~~~ function tools/freeze`);
}
