...
export function freeze(message, args) {
    ...
    if (!args.length) return Commands.run(`tellraw "${player.nameTag}" {"rawtext":[{"text":"§r§6[§aParadox§6]§r "},{"text":"You need to provide which target to freeze!"}]}`, World.getDimension("overworld"));
    ...
    return Commands.run(`execute "${member.nameTag}" ~~~ function tools/freeze`, World.getDimension("overworld"));
}
