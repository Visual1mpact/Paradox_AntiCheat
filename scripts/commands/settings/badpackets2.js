 ...
 */
export function badpackets2(message) {
    ...
    if (config.modules.badpackets2.enabled === false) {
        // Allow
        config.modules.badpackets2.enabled = true;
        player.runCommand(`tellraw @a[tag=paradoxOpped] {"rawtext":[{"text":"\n§r§4[§6Paradox§4]§r "},{"selector":"@s"},{"text":" has enabled §6Badpackets2§r!"}]}`);
        return;
    } else if (config.modules.badpackets2.enabled === true) {
        // Deny
        config.modules.badpackets2.enabled = false;
