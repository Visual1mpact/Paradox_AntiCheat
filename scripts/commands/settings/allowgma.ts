...
function allowgmaHelp(
  player: Player,
  prefix: string,
  adventureGMBoolean: string | number | boolean
) {
...
export function allowgma(message: BeforeChatEvent, args: string) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/allowGMA.js:36)"
    );
  }
  ...
  if (adventureGMBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disallowed §4Gamemode 2 (Adventure)§r to be used!`
    );
    Adventure();
    ...
    return sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has allowed §6Gamemode 2 (Adventure)§r to be used!`
    );
  }
}
