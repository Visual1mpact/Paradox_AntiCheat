...
function antiteleportHelp(
  player: Player,
  prefix: string,
  antiTeleportBoolean: string | number | boolean
) {
...
export function antiteleport(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/antiteleport.js:36)"
    );
  }
  ...
  if (antiTeleportBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6Anti Teleport§r!`
    );
    AntiTeleport();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4Anti Teleport§r!`
    );
    return;
  }
}
