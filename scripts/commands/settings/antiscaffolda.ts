...
function antiscaffoldaHelp(
  player: Player,
  prefix: string,
  antiScaffoldABoolean: string | number | boolean
) {
...
export function antiscaffoldA(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/antiscaffolda.js:36)"
    );
  }
  ...
  if (antiScaffoldABoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6AntiScaffoldA§r!`
    );
    ScaffoldA();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4AntiScaffoldA§r!`
    );
    return;
  }
}
