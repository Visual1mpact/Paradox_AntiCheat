...
function noslowAHelp(
  player: Player,
  prefix: string,
  noSlowBoolean: string | number | boolean
) {
...
export function noslowA(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/noslowa.js:36)"
    );
  }
  ...
  if (noSlowBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6NoSlowA§r!`
    );
    NoSlowA();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4NoSlowA§r!`
    );
    return;
  }
}
