...
export function worldborder(message) {
    ...
    if (!message) {
        return console.warn(`${new Date()} | ` + "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/worldborder.js:7)");
    }
    ...
    return player.dimension.runCommand(`execute "${player.nameTag}" ~~~ function settings/worldborder`);
}
