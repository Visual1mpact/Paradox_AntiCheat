...
function crasheraHelp(
  player: Player,
  prefix: string,
  crasherABoolean: string | number | boolean
) {
...
export function crasherA(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/crashera.js:36)"
    );
  }
  ...
  if (
    ...
    !config.customcommands.crashera
  ) {
  ...
  if (crasherABoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6CrasherA§r!`
    );
    CrasherA();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4CrasherA§r!`
    );
    return;
  }
}
