...
export function illegalitemsB(message, args) {
    ...
    if (illegalItemsBBoolean === false) {
        ...
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6IllegalItemsB§r!`)
        IllegalItemsB();
        return;
    } else if (illegalItemsBBoolean === true) {
        // Deny
        World.setDynamicProperty('illegalitemsb_b', false);
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4IllegalItemsB§r!`)
        return;
    }
}
