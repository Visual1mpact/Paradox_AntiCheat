...
export function crasherA(message, args) {
    ...
    if (crasherABoolean === false) {
        ...
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6CrasherA§r!`)
        CrasherA();
        return;
    } else if (crasherABoolean === true) {
        // Deny
        World.setDynamicProperty('crashera_b', false);
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4CrasherA§r!`)
        return;
    }
}
