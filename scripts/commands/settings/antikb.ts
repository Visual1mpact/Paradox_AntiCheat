...
function antikbHelp(
  player: Player,
  prefix: string,
  antikbBoolean: string | number | boolean
) {
...
export function antiknockback(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/antikb.js:36)"
    );
  }
  ...
  if (antikbscore <= 0) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6Anti Knockback§r!`
    );
    AntiKnockbackA();
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4Anti Knockback§r!`
    );
  }
