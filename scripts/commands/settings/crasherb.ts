...
function crasherbHelp(
  player: Player,
  prefix: string,
  crasherBBoolean: string | number | boolean
) {
...
export function crasherB(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/crasherb.js:36)"
    );
  }
  ...
  if (
    ...
    !config.customcommands.crasherb
  ) {
  ...
  if (crasherBBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6CrasherB§r!`
    );
    CrasherB();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4CrasherB§r!`
    );
    return;
  }
}
