...
function spammerDHelp(
  player: Player,
  prefix: string,
  spammerDBoolean: string | number | boolean
) {
...
export function spammerD(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/spammerD.js:36)"
    );
  }
  ...
  if (spammerDBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6SpammerD§r!`
    );
    SpammerD();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4SpammerD§r!`
    );
    return;
  }
}
