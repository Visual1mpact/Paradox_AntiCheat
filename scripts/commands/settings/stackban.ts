...
function stackBanHelp(
  player: Player,
  prefix: string,
  stackBanBoolean: string | number | boolean
) {
...
export function stackban(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/stackban.js:35)"
    );
  }
  ...
  if (!illegalItemsABoolean && !illegalItemsBBoolean && !illegalItemsCBoolean) {
    ...
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r You need to enable Illegal Items to use this feature.`
    );
  }
  ...
  if (stackBanBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6StackBans§r!`
    );
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4StackBans§r!`
    );
    return;
  }
}
