...
function clearlagHelp(
  player: Player,
  prefix: string,
  clearLagBoolean: string | number | boolean
) {
...
export function clearlag(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/clearlag.js:36)"
    );
  }
  ...
  if (clearLagBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6ClearLag§r!`
    );
    ClearLag();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4ClearLag§r!`
    );
    return;
  }
}
