...
function autoauraHelp(
  player: Player,
  prefix: string,
  autoaurascore: string | number | boolean
) {
...
export function autokillaura(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/autoaura.js:33)"
    );
  }
  ...
  if (autoaurascore <= 0) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6Autoaura§r!`
    );
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4Autoaura§r!`
    );
  }
