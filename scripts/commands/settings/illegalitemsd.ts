...
function illegalItemsDHelp(
  player: Player,
  prefix: string,
  illegalItemsDBoolean: string | number | boolean
) {
...
export function illegalitemsD(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/illegalitemsd.js:36)"
    );
  }
  ...
  if (illegalItemsDBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6IllegalItemsD§r!`
    );
    IllegalItemsD();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4IllegalItemsD§r!`
    );
    return;
  }
}
