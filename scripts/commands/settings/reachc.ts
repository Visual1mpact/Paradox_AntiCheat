...
function reachCHelp(
  player: Player,
  prefix: string,
  reachCBoolean: string | number | boolean
) {
...
export function reachC(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/reachc.js:36)"
    );
  }
  ...
  if (reachCBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6ReachC§r!`
    );
    ReachC();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4ReachC§r!`
    );
    return;
  }
}
