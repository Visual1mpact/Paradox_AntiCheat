...
function namespoofBHelp(
  player: Player,
  prefix: string,
  nameSpoofBoolean: string | number | boolean
) {
...
export function namespoofB(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/namespoofb.js:36)"
    );
  }
  ...
  if (nameSpoofBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6NamespoofB§r!`
    );
    NamespoofB();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4NamespoofB§r!`
    );
    return;
  }
}
