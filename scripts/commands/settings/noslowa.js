...
export function noslowA(message, args) {
    ...
    if (noSlowBoolean === false) {
        ...
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6NoSlowA§r!`)
        NoSlowA();
        return;
    } else if (noSlowBoolean === true) {
        // Deny
        World.setDynamicProperty('noslowa_b', false);
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4NoSlowA§r!`)
        return;
    }
}
