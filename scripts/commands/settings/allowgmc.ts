...
function allowgmcHelp(
  player: Player,
  prefix: string,
  creativeGMBoolean: string | number | boolean
) {
...
export function allowgmc(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/allowGMC.js:37)"
    );
  }
  ...
  if (creativeGMBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disallowed §4Gamemode 1 (Creative)§r to be used!`
    );
    Creative();
    return;
    ...
    return sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has allowed §6Gamemode 1 (Creative)§r to be used!`
    );
  }
}
