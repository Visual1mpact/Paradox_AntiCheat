...
function invalidSprintAHelp(
  player: Player,
  prefix: string,
  invalidSprintABoolean: string | number | boolean
) {
...
export function invalidsprintA(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/invaidsprinta.js:36)"
    );
  }
  ...
  if (invalidSprintABoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6InvalidSprintA§r!`
    );
    InvalidSprintA();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4InvalidSprintA§r!`
    );
    return;
  }
}
