...
function illegalItemsCHelp(
  player: Player,
  prefix: string,
  illegalItemsCBoolean: string | number | boolean
) {
...
export function illegalitemsC(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/illegalitemsc.js:36)"
    );
  }
  ...
  if (illegalItemsCBoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6IllegalItemsC§r!`
    );
    IllegalItemsC();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4IllegalItemsC§r!`
    );
    return;
  }
}
