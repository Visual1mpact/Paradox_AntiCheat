...
function reachAHelp(
  player: Player,
  prefix: string,
  reachABoolean: string | number | boolean
) {
...
export function reachA(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/reacha.js:36)"
    );
  }
  ...
  if (reachABoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6ReachA§r!`
    );
    ReachA();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4ReachA§r!`
    );
    return;
  }
}
