...
function flyaHelp(
  player: Player,
  prefix: string,
  flyABoolean: string | number | boolean
) {
...
export function flyA(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/flya.js:36)"
    );
  }
  ...
  if (flyABoolean === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6FlyA§r!`
    );
    FlyA();
    return;
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4FlyA§r!`
    );
    return;
  }
}
