...
function enchantedArmorHelp(
  player: Player,
  prefix: string,
  encharmorscore: string | number | boolean
) {
...
export function enchantedarmor(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/enchantedarmor.js:33)"
    );
  }
  ...
  if (encharmorscore <= 0) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6Anti Enchanted Armor§r!`
    );
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4Anti Enchanted Armor§r!`
    );
  }
