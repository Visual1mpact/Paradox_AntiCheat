...
export function jesusA(message, args) {
    ...
    if (jesusaBoolean === false) {
        ...
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6JesusA§r!`)
        JesusA();
        return;
    } else if (jesusaBoolean === true) {
        // Deny
        World.setDynamicProperty('jesusa_b', false);
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4JesusA§r!`)
        return;
    }
}
