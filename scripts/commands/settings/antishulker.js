...
export function antishulker(message, args) {
    ...
    if (antiShulkerBoolean === false) {
        ...
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6Anti-Shulkers§r!`)
        return;
    } else if (antiShulkerBoolean === true) {
        // Deny
        World.setDynamicProperty('antishulker_b', false);
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4Anti-Shulkers§r!`)
        return;
    }
}
