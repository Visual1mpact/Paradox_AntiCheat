...
function worldBorderHelp(
  player: Player,
  prefix: string,
  worldBorderBoolean: string | number | boolean
) {
...
export function worldborders(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/settings/worldborder.js:38)"
    );
  }
  ...
  if (argCheck === "disable") {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled the §6World Border§r!`
    );
   ...
   */
  if (
    (args[0] === "-overworld" || args[0] === "-o") &&
    isNaN(Number(args[1])) === false &&
    (args[2] === "-nether" || args[2] === "-n") &&
    isNaN(Number(args[3])) === false
  ) {
    // Build the wall
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has set the §6World Border§r! Overworld: ${args[1]} Nether: ${args[3]}`
    );
   ...
   */
  if (
    (args[0] === "-nether" || args[0] === "-n") &&
    isNaN(Number(args[1])) === false &&
    (args[2] === "-overworld" || args[2] === "-o") &&
    isNaN(Number(args[3])) === false
  ) {
    // Build the wall
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has set the §6World Border§r! Overworld: ${args[3]} Nether: ${args[1]}`
    );
    World.setDynamicProperty("worldborder_b", true);
  ...
  if (isNaN(Number(args[0])) === false && isNaN(Number(args[1])) === false) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has set the §6World Border§r! Overworld: ${args[0]} Nether: ${args[1]}`
    );
    World.setDynamicProperty("worldborder_b", true);
  ...
  if (
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has set the §6World Border§r! Nether: ${args[1]}`
    );
    World.setDynamicProperty("worldborder_b", true);
    World.setDynamicProperty("worldborder_nether_n", Math.abs(Number(args[1])));
    WorldBorder();
  ...
  if (
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r has set the §6World Border§r! Overworld: ${args[1]}`
    );
    World.setDynamicProperty("worldborder_b", true);
    World.setDynamicProperty("worldborder_n", Math.abs(Number(args[1])));
    WorldBorder();
