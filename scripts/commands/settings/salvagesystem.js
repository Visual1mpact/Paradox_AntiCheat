...
export function salvage(message, args) {
    ...
    if (salvageBoolean === false) {
        ...
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has enabled §6Salvage§r!`)
        return;
    } else if (salvageBoolean === true) {
        // Deny
        World.setDynamicProperty('salvage_b', false);
        sendMsgToPlayer('@a[tag=paradoxOpped]', `§r§4[§6Paradox§4]§r ${player.nameTag}§r has disabled §4Salvage§r!`)
        return;
    }
}
