...
function listQueue(queue: string, player: Player) {
  if (queue) {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r ${player.nameTag}§r is queued to be unbanned`
    );
  }
}
...
export function unban(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/moderation/unban.js:35)"
    );
  }
  ...
  if (
    ...
    (argCheck && args[0].toLowerCase() === "list") ||
    !config.customcommands.unban
  ) {
  ...
  sendMsg(
    "@a[tag=paradoxOpped]",
    `§r§4[§6Paradox§4]§r ${args
      .join(" ")
      .replace(regexp, "")} is queued to be unbanned!`
  );
}
