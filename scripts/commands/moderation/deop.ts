...
export function deop(message: BeforeChatEvent, args: string[]) {
  ...
  if (memberHash !== undefined && memberHash === memberEncode) {
    ...
    sendMsg(
      "@a[tag=paradoxOpped]",
      `§r§4[§6Paradox§4]§r ${member.nameTag} is no longer Paradox-Opped.`
    );
    return sendMsgToPlayer(
      member,
      `§r§4[§6Paradox§4]§r Your OP status has been revoked!`
    );
  }
  return sendMsgToPlayer(
    player,
    `§r§4[§6Paradox§4]§r ${member.nameTag} never had permission to use Paradox.`
  );
}
