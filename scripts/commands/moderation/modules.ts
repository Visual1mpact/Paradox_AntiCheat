...
export function modules(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/moderation/modules.js:28)"
    );
  }
  ...
  const status = (b: string | number | boolean) =>
    b ? "§aENABLED" : "§4DISABLED";
  ...
  sendMsgToPlayer(player, [
    ...
    `IllegalItemsA: ${
      illegalItemsABoolean
        ? `§aENABLED§r [Ban Illegal Stacks ${status(stackBanBoolean)}§r]`
        : "§4DISABLED"
    }`,
    `IllegalItemsB: ${
      illegalItemsBBoolean
        ? `§aENABLED§r [Ban Illegal Stacks ${status(stackBanBoolean)}§r]`
        : "§4DISABLED"
    }`,
    `IllegalItemsC: ${
    ...
    `World Border: ${
      worldBorderBoolean
        ? `§aENABLED§r (Overworld: ${worldBorderOverworldNumber} Nether: ${worldBorderNetherNumber})`
        : "§4DISABLED"
    }`,
