...
export function prefix(message: BeforeChatEvent, args: string[]) {
  ...
  if (args[0][0] == "/") {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r Using prefix '/' is not allowed!`
    );
  }
  ...
  if (args[0].length <= 2 && args[0].length >= 1) {
    ...
    sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r Prefix has been changed to '${args[0]}'!`
    );
    return player.addTag("Prefix:" + args[0]);
  } else {
    sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r Prefix length cannot be more than 2 characters!`
    );
  }
}
