...
export function punish(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/moderation/punish.js:10)"
    );
  }
  ...
  if (member === player) {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r You cannot punish yourself.`
    );
  }
  ...
  let inventoryContainer = member.getComponent(
    "minecraft:inventory"
  ...
  sendMsgToPlayer(
    member,
    `§r§4[§6Paradox§4]§r You have been punished for your behavior!`
  );
  // Use try/catch in case nobody has tag 'notify' as this will report 'no target selector'
  return sendMsg(
    "@a[tag=paradoxOpped]",
    `§r§4[§6Paradox§4]§r ${player.nameTag}§r has punished ${member.nameTag}§r`
  );
}
