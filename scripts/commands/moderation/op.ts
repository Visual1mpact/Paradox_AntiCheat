...
export function op(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/moderation/op.js:30)"
    );
  }
  ...
  if (
    hash === undefined ||
    (hash !== encode && args[0] !== config.modules.encryption.password)
  ) {
  ...
  sendMsg(
    "@a[tag=paradoxOpped]",
    `§r§4[§6Paradox§4]§r ${member.nameTag}§r is now Paradox-Opped.`
  );
