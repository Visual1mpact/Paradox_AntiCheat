...
export function kick(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/moderation/kick.js:33)"
    );
  }
  ...
  let reason =
    args.slice(1).join(" ").replace("-s", "") || "No reason specified";
  ...
  if (member === player) {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r You cannot kick yourself.`
    );
  }
  ...
  try {
    ...
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r I was unable to kick that player! Error: ${error}`
    );
  }
  return sendMsg(
    "@a[tag=paradoxOpped]",
    `§r§4[§6Paradox§4]§r ${player.nameTag}§r has kicked ${member.nameTag}§r. Reason: ${reason}`
  );
}
