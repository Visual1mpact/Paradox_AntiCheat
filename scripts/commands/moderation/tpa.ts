...
export function tpa(message: BeforeChatEvent, args: string[]) {
  ...
  if (!message) {
    return console.warn(
      `${new Date()} | ` +
        "Error: ${message} isnt defined. Did you forget to pass it? (./commands/moderation/tpa.js:31)"
    );
  }
  ...
  for (let pl of World.getPlayers()) {
    ...
    if (
      pl.nameTag
        .toLowerCase()
        .includes(args[1].toLowerCase().replace(/"|\\|@/g, ""))
    ) {
      member = pl;
    }
  ...
  if (!member) {
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r Couldnt find that player! Try '${prefix}tpa help' for more info.`
    );
  }
  ...
  if (args[0] && args[1]) {
    ...
    return sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r Teleported ${artificalPlayer.name} to ${member.name}`
    );
  } else {
    // Need to specify who
    sendMsgToPlayer(
      player,
      `§r§4[§6Paradox§4]§r You forgot to mention 'from' and 'who' to teleport.`
    );
    return tpaHelp(player, prefix);
  }
}
